 ...
 *
 * Metadata only: what it is called, who reads it and how long it runs. Where the audio comes from
 ...
 * `core:sound:manifest`.
 *
 * The invariants are checked here rather than trusted, because the list this model is built from
 * will eventually be a parsed API response. A story with no title is a row a screen would render as
 * a blank card; a negative duration is a progress bar that cannot be drawn.
 */
data class Story(
    val id: StoryId,
    val title: String,
