 ...
 */
class ManifestSoundSourceAdapterTest {
    private val adapter = ManifestSoundSourceAdapter(
        listOf(
            CatalogEntry(
    ...
    fun aTrackIsAnsweredWithItsCacheNameAndItsSource() {
        val source = assertNotNull(adapter.sourceFor(TrackId("heavy-rain")))
    ...
    fun anUnknownTrackHasNoSource() {
        assertNull(adapter.sourceFor(TrackId("no-such-track")))
    }

    @Test
    fun theNamesToKeepAreExactlyWhatTheEntriesReferTo() {
        assertEquals(setOf("heavy-rain-v1.mp3", "calm-waves-v2.mp3"), adapter.cacheFileNames)
    }
    ...
    fun everyShippedTrackIsReachableThroughThePort() {
        val shipped = ManifestSoundSourceAdapter()
