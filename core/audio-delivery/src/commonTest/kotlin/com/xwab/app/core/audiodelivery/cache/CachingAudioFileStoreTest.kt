...
class CachingAudioFileStoreTest {
    ...
    fun aCompletedDownloadClearsFilesTheCatalogNoLongerRefersTo() = runBlocking {
        ...
        store(directory, FakeTransport(directory), catalogKeeping(FILE_NAME, kept))
            .download(FILE_NAME, REMOTE_URL)
    ...
    fun aCancelledTransferStillClearsWhatItStaged() = runBlocking {
        ...
        val download = launch(Dispatchers.Default) {
            CachingAudioFileStore(directory, transport, catalogKeeping(FILE_NAME))
                .download(FILE_NAME, REMOTE_URL)
        }
        staged.await()
    ...
    ) = CachingAudioFileStore(directory, transport, sourceCatalog)
