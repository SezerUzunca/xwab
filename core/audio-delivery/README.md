...
`core:catalog-manifest`'s answer, read through `AudioSourceCatalog`. Nothing here holds a copy of
the manifest.

## Nobody but playback declares this module

`core:audio-delivery` appears in exactly two build files: `core:playback-session`, which assembles a
