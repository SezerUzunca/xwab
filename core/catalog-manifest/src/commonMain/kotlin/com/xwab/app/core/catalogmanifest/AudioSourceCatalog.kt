...
/**
 * The manifest's second port: the physical source behind each track, read by `core:audio-delivery`
 * and by nothing else.
 *
 * Kept apart from `MusicCatalogRepository` because the two answer to different callers — and kept
 * in a different *module* from it for the same reason. A screen depends on `core:catalog` for the
