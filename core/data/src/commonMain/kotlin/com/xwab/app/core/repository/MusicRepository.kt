...
interface MusicRepository {
    ...
    fun getMusicForCategory(categoryId: String): Flow<List<Music>>
    fun getMusic(musicId: String): Flow<Music?>
}

internal class LocalMusicRepository : MusicRepository {
    ...
    private val categories = categoryDefinitions.map { category ->
        category.copy(musicCount = musics.count { it.categoryId == category.id })
    }
    private val categoryStream = flowOf(categories)
    private val musicStream = flowOf(musics)

    override fun getCategories(): Flow<List<Category>> = categoryStream
    override fun getAllMusic(): Flow<List<Music>> = musicStream
    override fun getCategory(categoryId: String): Flow<Category?> = categoryStream.map { categories -> categories.find { it.id == categoryId } }
    override fun getMusicForCategory(categoryId: String): Flow<List<Music>> = musicStream.map { musics -> musics.filter { it.categoryId == categoryId } }
    override fun getMusic(musicId: String): Flow<Music?> = musicStream.map { musics -> musics.find { it.id == musicId } }
}
