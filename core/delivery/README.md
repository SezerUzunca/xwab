...
- `SoundPort` supplies the HTTPS URL and stable cache filename for a `TrackId`.
...
`NetworkPort` plus `SoundPort` at compile time.
...
After successful promotion, cached files no longer named by `SoundPort.cacheFileNames` are
